define(['gamejs', 'modules/ai/characters/enemy', 'modules/globals', 'modules/objects/weapon', 'modules/helpers/utils', 'gamejs/utils/math', 'gamejs/utils/vectors'], function(gamejs, enemy, globals, weapon, utils, $m, $v) {
     ...
     */
    var Ufo = function(level, spriteUrl, speed, life, fireRate) {
        Ufo.superConstructor.apply(this, arguments);

        // init arguments
        spriteUrl = spriteUrl || globals.enemies.images.Ufo;
    ...
    gamejs.utils.objects.extend(Ufo, enemy.ShooterEnemy);
    ...
    Ufo.prototype.setInitialPosition = function() {
    ...
    Ufo.prototype.update = function(msDuration) {
    ...
    /*
     * HeavyUfo ship.
     * Slightly more intelligent Ufos that fire more often than a simple
     * Ufo and move in a semi random direction with a semi random rotation
     * speed.
     */
    var HeavyUfo = function(level) {
        var args = [].splice.call(arguments, 0);
        HeavyUfo.superConstructor.apply(this, args.concat([
            globals.enemies.images.heavyUfo,
    ...
    gamejs.utils.objects.extend(HeavyUfo, Ufo);

    HeavyUfo.prototype.update = function(msDuration) {
        this.rect.left += this.speed * Math.cos(this.orientation - Math.PI/2) * ((Math.pow(this.level.speed, 2) / 10) + 1);
        this.rect.top  += this.speed * Math.sin(this.orientation - Math.PI/2) * ((Math.pow(this.level.speed, 2) / 10) + 1);
        if (this.rect.left < -this.image.getSize()[0] || this.rect.top < -this.image.getSize()[1] || this.rect.top > globals.game.screenSize[1])
            this.kill();
